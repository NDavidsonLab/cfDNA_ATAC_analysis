#!/usr/bin/env python3
"""
libnorm85_cfdna_counts.py

Per-sample 85% quantile library normalization for cfDNA Subnuc-count matrix
(non-negative integer counts, NOT SE values).

For each sample, divide column by 85th percentile of non-zero values.
This is the same procedure as ATAC libnorm85 (not |value| since values
are already non-negative).

After normalization, each sample's q85 of non-zero counts = 1.0.

Input:  cfdna_subnuc_count_per_peak.tsv.gz
Output: cfdna_subnuc_count_per_peak_libnorm85.tsv.gz
        libnorm85_cfdna_counts_scaling_factors.tsv
"""
...
def main() -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(args.output_matrix.parent /
                                  "libnorm85_cfdna_counts.log", mode="w"),
    ...
    for j, col_name in enumerate(M.columns):
        col = M.iloc[:, j].values
        # Counts are non-negative; "non-zero" means > 0
        nonzero = col[col > 0]
        nonzero_counts[j] = len(nonzero)
        if len(nonzero) == 0:
            logging.warning(f"  {col_name}: NO non-zero counts "
